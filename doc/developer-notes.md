...
Alarmx Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadSocketHandler : Sends/Receives data from peers on port 2579.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 2578 for connections and services them.
...
However, in open source software such as Alarmx Core, where everyone uses
...
pay attention to for reviewers of Alarmx Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Alarmx Core uses tinyformat, which is type safe. Leave them out to avoid confusion
