...
const char * const BITCOIN_CONF_FILENAME = "alarmx.conf";
const char * const BITCOIN_PID_FILENAME = "alarmxd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "alarmx" is a composite category enabling all Alarmx-related debug output
            if(ptrCategory->count(string("alarmx"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AlarmxCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AlarmxCore
    // Mac: ~/Library/Application Support/AlarmxCore
    // Unix: ~/.alarmxcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AlarmxCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AlarmxCore";
#else
    // Unix
    return pathRet / ".alarmxcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty alarmx.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override alarmx.conf
